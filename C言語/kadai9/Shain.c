...
/*引数(ShainData)で指定された社員情報をｷｬﾗｸﾀｺｰﾄﾞ(テキスト形式)に変換し、
　引数(F_Name)で指定されたファイルに書込む*/
char WINAPI Dll_TextChange(char F_Name[],char MaxShain,SHAIN_DATA ShainData[])
{
	SHAIN_CHAR Temp[MAX_SHAIN];
	/*以下の処理を社員数分、ShainDataのﾃﾞｰﾀをｷｬﾗｸﾀ変換*/
	for(i=0; i<MaxShain; i++)
	{
		...
		if(ShainData[i].Seibetsu==MAN)
		{
			strcpy_s(Temp[i].Seibetsu,sizeof(Temp[i].Seibetsu),"男性");
		}
		else
		{
			strcpy_s(Temp[i].Seibetsu,sizeof(Temp[i].Seibetsu),"女性");
		}
		switch (ShainData[i].Section)
    	{
			case EMPLOYER:
			strcpy_s(Temp[i].Section,sizeof(Temp[i].Section),"取締役");
			break;

			case GEBERAL:
			strcpy_s(Temp[i].Section,sizeof(Temp[i].Section),"総務部");
			break;

			case SALES:
			strcpy_s(Temp[i].Section,sizeof(Temp[i].Section),"営業部");
			break;
		}
	}
	...
	for(i=0; i<MaxShain; i++)
	{
		 /*Tempの社員ﾃﾞｰﾀを1人分、Kadai23.Datに書込む*/
		 kekka = Dll_FileWrite(F_Name,&Temp[i],sizeof(SHAIN_CHAR),i);
		 if(kekka==NG)
		 {
			 /*FileWrite関数の処理結果がNGなら戻り値にNGを設定する*/
			 return(NG);
		 }
	}
		return(OK);
