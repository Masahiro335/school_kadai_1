...
class Sample4_5_1 extends Frame implements ActionListener
{
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(item==m1_1)					//ｻﾌﾞﾒﾆｭｰが選択された際の処理
		{
			label.setText("新規作成→ﾌｫﾙﾀﾞが選択されました");
			m1_1.setEnabled(false);		//ｻﾌﾞﾒﾆｭｰのﾌｫﾙﾀﾞを無効にする
			m1_2.setEnabled(true);		//ｻﾌﾞﾒﾆｭｰのﾌｧｲﾙを有効にする
		}
		else if(item==m1_2)					//ｻﾌﾞﾒﾆｭｰが選択された際の処理
		{
			label.setText("新規作成→ﾌｫﾙﾀﾞが選択されました");
			m1_1.setEnabled(true);		//ｻﾌﾞﾒﾆｭｰのﾌｫﾙﾀﾞを有効にする
			m1_2.setEnabled(false);		//ｻﾌﾞﾒﾆｭｰのﾌｧｲﾙを無効にする
		}
